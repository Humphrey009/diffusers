...
# 启动训练脚本
start_time=$(date +%s)
nohup python3 -m torch.distributed.run --nproc_per_node 8 train_text_to_image_sdxl.py \
  ...
  --output_dir ./output > ${scripts_path_dir}/output/run_text_to_image_sdxl.log 2>&1 &
wait
end_time=$(date +%s)
