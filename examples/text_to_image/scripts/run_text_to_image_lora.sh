...
# 启动训练脚本
start_time=$(date +%s)
nohup python3 -m torch.distributed.run --nproc_per_node 8 train_text_to_image_lora.py \
  ...
  --output_dir ./output_text_to_image_lora > ${scripts_path_dir}/output_text_to_image_lora/run_text_to_image_lora.log 2>&1 &
wait
end_time=$(date +%s)
